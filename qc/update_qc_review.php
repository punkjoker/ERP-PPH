...
<section class="mb-8">
    ...
    <!-- ✅ QC Inspection Form -->
<form action="save_qc_review.php" method="POST"
  x-data="{
    tests: [],
  ...
  <div class="bg-white p-6 rounded-lg shadow mb-6">
    ...
    <table class="min-w-full text-sm border">
      ...
      <tbody>
        <?php
// ✅ Fetch existing review data for this run (if any)
$existing_reviews = [];
$review_query = $conn->prepare("SELECT checklist_no, response FROM quality_manager_review WHERE production_run_id = ?");
$review_query->bind_param("i", $production['id']);
$review_query->execute();
$review_result = $review_query->get_result();
while ($r = $review_result->fetch_assoc()) {
    $existing_reviews[$r['checklist_no']] = $r['response'];
}
$review_query->close();
...
<tbody>
  <?php foreach ($checklist as $i => $item): 
    $response = $existing_reviews[$i] ?? ''; // ✅ Use previous response if exists
  ?>
  <tr>
    <td class="border px-2 py-1 text-center"><?= $i + 1 ?></td>
    <td class="border px-2 py-1"><?= htmlspecialchars($item) ?></td>
    <td class="border px-2 py-1 text-center">
      <input type="radio" name="checklist_<?= $i ?>" value="Yes" <?= ($response === 'Yes') ? 'checked' : '' ?>>
    </td>
    <td class="border px-2 py-1 text-center">
      <input type="radio" name="checklist_<?= $i ?>" value="No" <?= ($response === 'No') ? 'checked' : '' ?>>
    </td>
  </tr>
  <?php endforeach; ?>
</tbody>
  ...
  <div class="flex justify-end space-x-4">
    <a href="quality_manager_review.php" class="bg-gray-500 text-white px-6 py-2 rounded-lg shadow hover:bg-gray-600">
      ← Back
    </a>
