...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    $week_no    = date('W', strtotime($start_date));
...
<head>
    <meta charset="UTF-8">
    <title>Edit Lunch Expense</title>
...
<div class="ml-64 p-6 max-w-4xl mx-auto">
    <h1 class="text-3xl font-bold mb-6 text-blue-700">Edit Lunch Expense</h1>
    ...
    <div class="bg-white shadow rounded-lg p-6 mb-6">
        <form method="POST" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input type="date" name="start_date" value="<?= htmlspecialchars($expense['start_date']) ?>" class="border p-2 rounded focus:ring-2 focus:ring-blue-300" required>
                <input type="date" name="end_date" value="<?= htmlspecialchars($expense['end_date']) ?>" class="border p-2 rounded focus:ring-2 focus:ring-blue-300" required>
                ...
                <div id="items-container" class="md:col-span-2 flex flex-col gap-2">
                    <?php foreach($items as $i=>$it): ?>
                        <div class="flex gap-2 mb-2">
                            ...
                            <input type="number" step="0.01" name="costs[]" value="<?= $it['cost'] ?>" class="border p-2 rounded w-1/2 focus:ring-2 focus:ring-blue-300" required oninput="calculateTotal()">
                        </div>
                    <?php endforeach; ?>
                </div>
                ...
                <input type="text" name="items_bought_by" value="<?= htmlspecialchars($expense['items_bought_by']) ?>" placeholder="Items Bought By" class="border p-2 rounded w-full focus:ring-2 focus:ring-blue-300 md:col-span-1">
                <input type="number" step="0.01" name="transport_cost" id="transport_cost" value="<?= $expense['transport_cost'] ?>" placeholder="Transport Cost" class="border p-2 rounded w-full focus:ring-2 focus:ring-blue-300 md:col-span-1" oninput="calculateTotal()">
            </div>
