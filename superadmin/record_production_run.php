        ...
        <table class="min-w-full border border-gray-200 text-sm">
          ...
          <tbody class="text-gray-700">
            <?php while ($row = $result->fetch_assoc()): ?>
              <?php
                $prod_status = $row['production_status'];
                if ($prod_status === 'Completed') {
                    $status_text = "<span class='text-green-600 font-semibold'>Completed</span>";
                } else {
                    $status_text = "<span class='text-yellow-600 font-semibold'>In Production</span>";
                }
              ?>
              <tr class="hover:bg-gray-50 transition">
                ...
                <td class="py-2 px-3 border space-x-2">
                  <a href="update_production.php?id=<?php echo $row['id']; ?>" 
                     class="bg-green-500 text-white px-3 py-1 rounded hover:bg-green-600 text-xs">Update</a>
