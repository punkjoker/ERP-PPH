...
<div class="ml-64 p-6 max-w-4xl">
    ...
    if(isset($_POST['submit'])) {
        ...
        $chemical_name = $_POST['chemical_name'] ?? null;
        $main_category = $_POST['main_category'] ?? null;
        $group_name = $_POST['group_name'] ?? null;
        $group_code = $_POST['group_code'] ?? null;
        $chemical_code = $_POST['chemical_code'] ?? null;
        $category = $_POST['category'] ?? null;
        $delivery_number = $_POST['delivery_number'] ?? null;
        $quantity_received = $_POST['quantity_received'] ?? null;
        ...
        $receiving_date = $_POST['receiving_date'] ?? null;

        $stmt = $conn->prepare("INSERT INTO store_b_chemicals_in 
            (chemical_id, chemical_name, main_category, group_name, group_code, chemical_code, category, delivery_number, quantity_received, units, pack_size, unit_cost, po_number, received_by, receiving_date) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("isssssssdssddsss",
            $chemical_id, $chemical_name, $main_category, $group_name, $group_code, $chemical_code, $category, $delivery_number, $quantity_received, $units, $pack_size, $unit_cost, $po_number, $received_by, $receiving_date
        );
        $stmt->execute();
...
$("#chemical_name").autocomplete({
    source: function(request, response) {
        $.getJSON("store_b_chemical_autocomplete.php", { term: request.term }, function(data) {
            response(data);
        });
    },
    minLength: 1,
