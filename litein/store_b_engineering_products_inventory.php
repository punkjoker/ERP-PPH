...
<head>
<meta charset="UTF-8">
<title>Store B Engineering Products Inventory</title>
...
<div class="ml-64 p-6 max-w-6xl">
    <h2 class="text-xl font-bold mb-4">Store B Engineering Products Inventory</h2>
    ...
    <form method="GET" class="mb-4 flex items-center space-x-2">
        ...
        <?php if (!empty($search)): ?>
            <a href="store_b_engineering_products_inventory.php" class="text-blue-600 underline">Clear</a>
        <?php endif; ?>
        <a href="download_store_b_engineering_list.php?search=<?= urlencode($_GET['search'] ?? '') ?>" 
   class="bg-green-500 text-white px-3 py-1 rounded hover:bg-green-600" 
   target="_blank">
   Download Engineering Products List
</a>
                ...
                if ($result && $result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo "<tr class='hover:bg-gray-50'>
                            ...
                            <td class='px-4 py-2'>
                                <a href='view_store_b_engineering_inventory.php?product_code=" . urlencode($row['product_code']) . "' 
                    ...
                    echo "<tr><td colspan='5' class='px-4 py-2 text-center'>No engineering products found.</td></tr>";
                }
                ?>
            </tbody>
        </table>
